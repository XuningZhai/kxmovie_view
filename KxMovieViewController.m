...
//        self.wantsFullScreenLayout = YES;
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            NSError *error = nil;
            [decoder openFileWithTCP:path error:&error];
...
- (void)didReceiveMemoryWarning
{
    ...
    if (self.playing) {
        ...
        if (_maxBufferedDuration > 0) {
            ...
            [_decoder closeFile];
            [_decoder openFileWithTCP:nil error:nil];
...
//    NSLog(@"_decoder.path = %@",_decoder.path);
//    NSLog(@"n = %ld",(long)[n integerValue]);
    if (n)
